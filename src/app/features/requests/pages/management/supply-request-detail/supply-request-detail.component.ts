...
export class SupplyRequestDetailComponent implements OnInit, OnDestroy {
  ...
  openAddItemModal(): void {
    ...
    this.allowedItemTypes = this.getAllowedItemTypes();
   ...
   */
  private getAllowedItemTypes(): number[] {
    ...
    if (this.orderData?.requestItems && this.orderData.requestItems.length > 0) {
      this.orderData.requestItems.forEach(item => {
    ...
    if (this.requestDetail?.items && this.requestDetail.items.length > 0) {
