  ...
  template: `
    <div class="flex flex-col h-full">
      ...
      <div class="grid grid-cols-2 xl:grid-cols-4 gap-3 mb-6">
        <div class="p-3 rounded-lg text-center border-2 border-blue-500/20 bg-blue-500/10 overflow-hidden">
          <div class="text-2xl font-bold text-blue-500 leading-none truncate">{{ metrics.newRequests }}</div>
          <div class="text-xs font-medium text-[var(--color-text-muted)] mt-2 truncate">{{ 'adminDashboard.requests.new' | translate }}</div>
        </div>

        <div class="p-3 rounded-lg text-center border-2 border-orange-500/20 bg-orange-500/10 overflow-hidden">
          <div class="text-2xl font-bold text-orange-500 leading-none truncate">{{ metrics.inProgressRequests }}</div>
          <div class="text-xs font-medium text-[var(--color-text-muted)] mt-2 truncate">{{ 'adminDashboard.requests.inProgress' | translate }}</div>
        </div>

        <div class="p-3 rounded-lg text-center border-2 border-green-500/20 bg-green-500/10 overflow-hidden">
          <div class="text-2xl font-bold text-green-500 leading-none truncate">{{ metrics.completedRequests }}</div>
          <div class="text-xs font-medium text-[var(--color-text-muted)] mt-2 truncate">{{ 'adminDashboard.requests.completed' | translate }}</div>
        </div>

        <div class="p-3 rounded-lg text-center border-2 border-red-500/20 bg-red-500/10 overflow-hidden">
          <div class="text-2xl font-bold text-red-500 leading-none truncate">{{ metrics.rejectedRequests }}</div>
