      ...
      >
          ...
          <div class="relative w-24 h-24">
            <svg class="w-24 h-24 -rotate-90" viewBox="0 0 100 100" aria-hidden="true">
              ...
              <circle
                ...
                cy="50"
                r="44"
                [attr.stroke]="idleState.secondsRemaining <= 10 ? 'var(--color-error)' : 'var(--color-brand)'"
                stroke-width="6"
                fill="none"
                stroke-linecap="round"
                [attr.stroke-dasharray]="circumference"
                [attr.stroke-dashoffset]="getStrokeDashoffset(idleState.secondsRemaining)"
            ...
            <div class="absolute inset-0 flex items-center justify-center">
              <span
                ...
                [style.color]="idleState.secondsRemaining <= 10 ? 'var(--color-error)' : 'var(--color-text)'"
              >
                {{ idleState.secondsRemaining }}
              </span>
            </div>
          </div>
        </div>
        ...
        <div class="text-center space-y-2">
          ...
          <p id="idle-timeout-msg" class="text-sm text-[var(--color-text-muted)] leading-relaxed">
            {{ 'common.idleTimeout.message' | translate : { seconds: idleState.secondsRemaining } }}
          </p>
        </div>
...
export class IdleTimeoutModalComponent implements OnInit, OnDestroy {
  idleState: IdleState = { isWarning: false, secondsRemaining: 0 };
  ...
  ngOnInit(): void {
    this.idleService.state$.pipe(takeUntil(this.destroy$)).subscribe(state => {
